...
<body>
  ...
  <hr>
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT transactionID,bookTitle,bookISBN,checkOut from transactions where customer_email = '$user' AND checkOut ='y'");
        $stmt->execute();
        $result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
        $array = $stmt->fetchAll();
        $i = 0;
        while ($i <= (count($array)-1)){
          $txID = $array[$i]['transactionID'];
          $array[$i]['checkOut'] = "<form action = 'return_book.php' method = 'POST' ><input type='hidden' id='tx_id' name='tx_id' value='$txID'><div class='button'> <button type='submit''>Return</button></div></form>";
          $i++;
        }
